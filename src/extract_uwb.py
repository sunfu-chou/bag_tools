# This filee is used to extract images from the bag file
# and save them to the folder "images"
# The images are saved as .jpg files
...
def extract_images(bag_file, bag_file_dir, args):
    # create sub folder for each bag file    
    ...
    print("Extracting images from {}".format(sub_time_dir))
    ...
    # print("Bag length: {}".format(bag_length))
...
def image_msg_to_cv2(msg):
    """Convert a sensor_msgs.msg.Image message to an OpenCV
    image.
    """
    bridge = CvBridge()
...
def main():
    ...
    if args.output_dir == "":
        args.output_dir = os.path.join(args.bag_file_root, "images")
    ...
    for i in tqdm(range(len(bag_files))):
        extract_images(bag_files[i], bag_file_dir[i], args)
